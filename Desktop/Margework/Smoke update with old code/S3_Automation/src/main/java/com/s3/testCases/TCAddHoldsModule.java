...
public class TCAddHoldsModule {
	public static int mRow;
	public static int mCol;

	String excelFile = Constant.EXCEL_FILE_NAME;
	String excelSheetName = Constant.ADD_HOLDSMODULE_SHEET;
	String filepath = Constant.REPORTS_HOLDSMODULE_FILE;
	ExtentReports extent = ExtentReports.get(TCAddHoldsModule.class);

	@SuppressWarnings("deprecation")
	@BeforeMethod()
	public void logConfi() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TCAddHoldsModule");
		Utils.OpenBrowser(Constant.BROWSER_NAME, Constant.URL);
		extent.init(filepath, true);
		extent.startTest("TCAddHoldsModule", "S3 TCAddHoldsModule Test Started");
	...
	public void addPreDefinedWebModule() throws Exception {
		...
		try {
			...
			extent.log(LogStatus.PASS, "TCAddHoldsModule", "Test Pass");
			...
			ExcelUtils.wrtExcelData(141, 3, statusExcel, statusSheet, "Pass");
			ExcelUtils.wrtExcelData(141, 6, statusExcel, statusSheet, "");
		} catch (Exception e) {
			try {
				...
				String statusExcel = Constant.STATUS_EXCEL_FILE;
				String statusSheet = Constant.STATUS_SHEET;
				ExcelUtils.wrtExcelData(141, 3, statusExcel, statusSheet, "Fail");
				ExcelUtils.wrtExcelData(141, 6, statusExcel, statusSheet, e.getMessage());
				Assert.assertTrue(false);
			} catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}
			e.getMessage();
			extent.log(LogStatus.FAIL, "TCAddHoldsModule", "Test Fail");
		}
	}
