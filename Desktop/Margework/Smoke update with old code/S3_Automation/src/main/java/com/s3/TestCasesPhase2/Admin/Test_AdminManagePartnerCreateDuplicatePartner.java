...
public class Test_AdminManagePartnerCreateDuplicatePartner {
	...
	public void Setup() throws Exception {
		...
		String runMode= BaseLogic.Runable(3, 2, 7, 3, Constant.RUNMODEADMIN);
		if(runMode.equals("N"))	{
		ExcelUtils.wrtExcelData(7, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, "Not Executed");
		ResultCol++;
		throw new Exception("RunMode off");
		}
		System.out.println("Verify duplicate partner not create with same Name");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TC_Admin_07");
	...
	public void ManagePartnerduplicatePartner(String Username, String Password, String inputText, String expectedmsg) throws FileNotFoundException {
		String Result ="Fail";
		try {
			Thread.sleep(1000);
			LoginPhase2.Login(Username, Password);
			Thread.sleep(2000);
			Result = AdminPhase2.ManagePartner_CreateDuplicatePartner(inputText, expectedmsg);
			System.out.println("Create new user Required fields verification is  " + Result);
			ExcelUtils.wrtExcelData(7, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			Assert.assertEquals(Result, "Pass");
		} catch (Exception e) {
			System.out.println("Test cases Fail");
			ExcelUtils.wrtExcelData(7, Constant.RESULT_COL + ResultCol, statusExcel, statusSheet, "Fail");
			ResultCol++;
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.MANPARTNERCREATEDUPLICATEPARTNER;
