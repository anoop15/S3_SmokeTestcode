...
public class TCAddVideosModule {
	public static int mRow;
	public static int mCol;

	String excelFile = Constant.EXCEL_FILE_NAME;
	String excelSheetName = Constant.ADD_VIDEOSMODULE_SHEET;
	String filepath = Constant.REPORTS_VIDEOSMODULE_FILE;
	ExtentReports extent = ExtentReports.get(TCAddVideosModule.class);

	@SuppressWarnings("deprecation")
	@BeforeMethod()
	public void logConfi() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("Videos Module");
		Utils.OpenBrowser(Constant.BROWSER_NAME, Constant.URL);
		extent.init(filepath, true);
		extent.startTest("Videos Module", "S3 Videos Test Started");
	...
	public void addNewsVideoSportsModule() throws Exception {
		...
		try {
			...
			extent.log(LogStatus.PASS, "News/Videos/Events/Sports test", "Test Pass");
			String statusExcel = Constant.STATUS_EXCEL_FILE;
			String statusSheet = Constant.STATUS_SHEET;
			ExcelUtils.wrtExcelData(163, 3, statusExcel, statusSheet, "Pass");
			ExcelUtils.wrtExcelData(163, 6, statusExcel, statusSheet, "");
		} catch (Exception e) {
			try {
				ExcelUtils.wrtExcelData(1, 27, excelFile, excelSheetName, "Fail");
				Utils.getScreenShot(Constant.OUTPUT_DIR + "addVideosFailScreenShot.PNG");
				extent.attachScreenshot(Constant.OUTPUT_DIR + "addVideosFailScreenShot.PNG");
				String statusExcel = Constant.STATUS_EXCEL_FILE;
				String statusSheet = Constant.STATUS_SHEET;
				ExcelUtils.wrtExcelData(163, 3, statusExcel, statusSheet, "Fail");
				ExcelUtils.wrtExcelData(163, 6, statusExcel, statusSheet, e.getMessage());
			...
			e.getMessage();
			extent.log(LogStatus.FAIL, "Videos test", "Test Fail");
		}
	}
