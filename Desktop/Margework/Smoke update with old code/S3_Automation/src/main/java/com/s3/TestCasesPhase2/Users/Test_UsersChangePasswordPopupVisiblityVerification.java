...
public class Test_UsersChangePasswordPopupVisiblityVerification {
	...
	public void Setup() throws Exception {
		String runMode= LoginPhase2.Runable(2, 2, 13, 3, Constant.RUNMODEUSER);
		if(runMode.equals("N"))	{
		...
		System.out.println("*****Change Password Popup visblity Verification*****");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("Search user, TC_User_13 ");
	...
	public void SearchUserVerification(String username, String password, String rememberme,String searchText) throws Exception {
		...
		try {
			...
			//Result1 = UserProfilePhase2.searchUser("Email", searchText);
			Result=UserProfilePhase2.changePasswordPopupVisiblity(searchText);
			ExcelUtils.wrtExcelData(13, Constant.RESULT_COL + ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			Assert.assertEquals(Result, "Pass");
			System.out.println(searchText + " User search sucessfully and all required fields are display on change password popup");
		} catch (Exception e) {
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.CHANGEPASSWORDPOPUP;
