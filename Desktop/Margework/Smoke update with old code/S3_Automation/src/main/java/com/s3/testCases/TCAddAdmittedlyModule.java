...
public class TCAddAdmittedlyModule {

	public static int mRow;
	public static int mCol;

	String excelFile = Constant.EXCEL_FILE_NAME;
	String excelSheetName = Constant.ADD_ADMITTEDLYMODULE_SHEET;
	String filepath = Constant.REPORTS_ADMITTEDLY_FILE;
	ExtentReports extent = ExtentReports.get(TCAddAdmittedlyModule.class);

	@SuppressWarnings("deprecation")
	@BeforeMethod()
	public void logConfi() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TCAddAdmittedlyModule");
		Utils.OpenBrowser(Constant.BROWSER_NAME, Constant.URL);
		extent.init(filepath, true);
		extent.startTest("TCAddAdmittedlyModule", "S3 TCAddAdmittedlyModule Test Started");
	...
	@Test()
	public void addAdmittedlyModule() throws Exception {
		...
		try {
			...
			extent.log(LogStatus.PASS, "TCAddAdmittedlyModule", "Test Pass");
			...
			ExcelUtils.wrtExcelData(72, 3, statusExcel, statusSheet, "Pass");
			ExcelUtils.wrtExcelData(72, 6, statusExcel, statusSheet, "");
		} catch (Exception e) {
			try {
				...
				String statusExcel = Constant.STATUS_EXCEL_FILE;
				String statusSheet = Constant.STATUS_SHEET;
				ExcelUtils.wrtExcelData(72, 3, statusExcel, statusSheet, "Fail");
				ExcelUtils.wrtExcelData(72, 6, statusExcel, statusSheet, e.getMessage());
				Assert.assertTrue(false);
			} catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}
			e.getMessage();
			extent.log(LogStatus.FAIL, "TCAddAdmittedlyModule", "Test Fail");
		}
	}
