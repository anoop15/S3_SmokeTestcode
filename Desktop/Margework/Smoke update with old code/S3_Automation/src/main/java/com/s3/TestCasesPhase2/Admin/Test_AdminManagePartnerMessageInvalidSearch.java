...
public class Test_AdminManagePartnerMessageInvalidSearch {
	...
	public void Setup() throws Exception {
		...
		String runMode= BaseLogic.Runable(3, 2, 12, 3, Constant.RUNMODEADMIN);
		if(runMode.equals("N"))	{
		...
		System.out.println("Verify Message when search non exiting partner");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TC_Admin_12");
	...
	public void ManagePartnerVerifyMessageonInvalidSearch(String Username, String Password,String Searchpartner) throws FileNotFoundException {
		String Result ="Fail";
		try {
			Thread.sleep(1000);
			...
			Thread.sleep(4000);
			Result=UserProfilePhase2.InvalidsearchUser(Searchpartner);
			System.out.println("Verify Message when search result not found in list " + Result);
			ExcelUtils.wrtExcelData(12, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			ExcelUtils.wrtExcelData(12, Constant.RESONE_COL, statusExcel, statusSheet,"");
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.MANPARTNERINVALIDSEARCH;
