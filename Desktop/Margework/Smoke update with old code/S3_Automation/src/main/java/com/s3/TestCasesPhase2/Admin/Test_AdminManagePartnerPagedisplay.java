...
public class Test_AdminManagePartnerPagedisplay {
	...
	public void Setup() throws Exception {
		...
		String runMode= BaseLogic.Runable(3, 2, 1, 3, Constant.RUNMODEADMIN);
		if(runMode.equals("N"))	{
		...
		System.out.println("Validte the Manage Partner page visiblity");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TC_Admin_01");
	...
	public void ManagePartnerPageVisiblity(String Username, String Password) throws FileNotFoundException {
		String Result ="Fail";
		try {
			Thread.sleep(1000);
			LoginPhase2.Login(Username, Password);
			Thread.sleep(2000);
			Result = AdminPhase2.ManagePartnerPageVisibility();
			System.out.println("Manage Patrner page display " + Result);
			ExcelUtils.wrtExcelData(1, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			Assert.assertEquals(Result, "Pass");
		} catch (Exception e) {
			Log.info("Manage partner page visiblity case fail");
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.MANAGEPARTNERPAGEVISIBLITY;
