...
public class Test_AdminManagePartnerValidSearchPartner {
	...
	public void Setup() throws Exception {
		...
		String runMode= BaseLogic.Runable(3, 2, 13, 3, Constant.RUNMODEADMIN);
		if(runMode.equals("N"))	{
		ExcelUtils.wrtExcelData(13, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, "Not Executed");
		ResultCol++;
		throw new Exception("RunMode off");
		}
		System.out.println("Verify Search partner functionality");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TC_Admin_13");
	...
	public void ManagePartnerVerifySearchPartner(String Username, String Password,String Searchpartner) throws FileNotFoundException {
		String Result ="Fail";
		try {
			Thread.sleep(1000);
			LoginPhase2.Login(Username, Password);
			Thread.sleep(2000);
			AdminPhase2.clkAdmin();
			Thread.sleep(4000);
			AdminPhase2.clkManagePartner();
			Thread.sleep(4000);
			Result= AdminPhase2.SearchPartner(Searchpartner);
			System.out.println("Verify Searched Partner display in partner list " + Result);
			ExcelUtils.wrtExcelData(13, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			ExcelUtils.wrtExcelData(13, Constant.RESONE_COL, statusExcel, statusSheet,"");
			Assert.assertEquals(Result, "Pass");
		} catch (Exception e) {
			System.out.println("Test cases Fail");
			System.out.println(e.getMessage());
			ExcelUtils.wrtExcelData(13, Constant.RESULT_COL + ResultCol, statusExcel, statusSheet, "Fail");
			ResultCol++;
			ExcelUtils.wrtExcelData(13, Constant.RESONE_COL, statusExcel, statusSheet,e.getMessage());
			Assert.assertEquals(Result, "Pass");
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.MANPARTNERVALIDSEARCH;
