...
public class TCAddTwitterModule {

	public static int mRow;
	public static int mCol;

	String excelFile = Constant.EXCEL_FILE_NAME;
	String excelSheetName = Constant.ADD_TWITTERMODULE_SHEET;
	String filepath = Constant.REPORTS_TWITTERMODULE_FILE;
	ExtentReports extent = ExtentReports.get(TCAddTwitterModule.class);

	@SuppressWarnings("deprecation")
	@BeforeMethod()
	public void logConfi() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TCAddTwitterModule");
		Utils.OpenBrowser(Constant.BROWSER_NAME, Constant.URL);
		extent.init(filepath, true);
		extent.startTest("TCAddTwitterModule", "S3 TCAddTwitterModule Test Started");
	...
	public void addPreDefinedWebModule() throws Exception {
		...
		try {
			...
			extent.log(LogStatus.PASS, "TCAddTwitterModule", "Test Pass");
			...
			ExcelUtils.wrtExcelData(151, 3, statusExcel, statusSheet, "Pass");
			ExcelUtils.wrtExcelData(151, 6, statusExcel, statusSheet,"");
		} catch (Exception e) {
			try {
				...
				String statusExcel = Constant.STATUS_EXCEL_FILE;
				String statusSheet = Constant.STATUS_SHEET;
				ExcelUtils.wrtExcelData(151, 3, statusExcel, statusSheet, "Fail");
				ExcelUtils.wrtExcelData(151, 6, statusExcel, statusSheet, e.getMessage());
				Assert.assertTrue(false);
			} catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}
			e.getMessage();
			extent.log(LogStatus.FAIL, "TCAddTwitterModule", "Test Fail");
		}
	}
