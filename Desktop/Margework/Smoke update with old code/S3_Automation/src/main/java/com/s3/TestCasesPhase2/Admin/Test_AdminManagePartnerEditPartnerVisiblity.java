...
public class Test_AdminManagePartnerEditPartnerVisiblity {
	...
	public void Setup() throws Exception {
		...
		String runMode= BaseLogic.Runable(3, 2, 15, 3, Constant.RUNMODEADMIN);
		if(runMode.equals("N"))	{
		ExcelUtils.wrtExcelData(15, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, "Not Executed");
		ResultCol++;
		throw new Exception("RunMode off");
		}
		System.out.println("Verify Edit partner popup visiblity");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TC_Admin_15");
	...
	public void ManagePartnerEditPartnervisiblity(String Username, String Password) throws FileNotFoundException {
		String Result ="Fail";
		try {
			Thread.sleep(1000);
			LoginPhase2.Login(Username, Password);
			Thread.sleep(2000);
			Result = AdminPhase2.ManagePartner_EditPartnerPopupVisiblity();
			System.out.println("Edit partner popup visiblity verification is  " + Result);
			ExcelUtils.wrtExcelData(15, Constant.RESULT_COL+ ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			Assert.assertEquals(Result, "Pass");
		} catch (Exception e) {
			System.out.println("Test cases Fail");
			ExcelUtils.wrtExcelData(15, Constant.RESULT_COL + ResultCol, statusExcel, statusSheet, "Fail");
			ResultCol++;
			Assert.assertEquals(Result, "NA");
			ExcelUtils.wrtExcelData(15, Constant.RESONE_COL, statusExcel, statusSheet,e.getMessage());
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.MANPARTNEREDITPARTNERPOPUP;
