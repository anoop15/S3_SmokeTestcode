...
public class Test_UsersCreateNewUser {
	...
	public void Setup() throws Exception {
		String runMode= LoginPhase2.Runable(2, 2, 5, 3, Constant.RUNMODEUSER);
		if(runMode.equals("N"))	{
		...
		System.out.println("Create new user verification");
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("Create new Admin user, TC_User_05 ");
	...
	public void CreateNewUserVerification(String username, String password, String rememberme,String firstName, String lastName,String email,
		...
		try {
			...
			Result = UserProfilePhase2.createNew_User(firstName,lastName,email,selectUsertype,partner,client,roles,emailfrequncey,action);
			ExcelUtils.wrtExcelData(5, Constant.RESULT_COL + ResultCol, statusExcel, statusSheet, Result);
			ResultCol++;
			Assert.assertEquals(Result, "Pass");
			System.out.println("New "+selectUsertype +" User Created sucessfully");
			String deleteuser=UserProfilePhase2.deleteuser(email,logintype);
		} catch (Exception e) {
			Log.info("New user not created successfully");
	...
	public Object[][] dataProviderMethod() throws Exception {
		...
		String strQuery = "Select * from " + Constant.NEWADMINUSER;
