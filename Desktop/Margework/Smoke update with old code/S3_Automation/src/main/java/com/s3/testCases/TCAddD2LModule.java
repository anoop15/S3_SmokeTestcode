...
public class TCAddD2LModule {

	public static int mRow;
	public static int mCol;

	String excelFile = Constant.EXCEL_FILE_NAME;
	String excelSheetName = Constant.ADD_D2LMODULE_SHEET;
	String filepath = Constant.REPORTS_D2LMODULE_FILE;
	ExtentReports extent = ExtentReports.get(TCAddD2LModule.class);

	@SuppressWarnings("deprecation")
	@BeforeMethod()
	public void logConfi() throws Exception {
		DOMConfigurator.configure("log4j.xml");
		Log.startTestCase("TCAddD2LModule");
		Utils.OpenBrowser(Constant.BROWSER_NAME, Constant.URL);
		extent.init(filepath, true);
		extent.startTest("TCAddD2LModule", "S3 TCAddD2LModule Test Started");
	...
	public void addPreDefinedWebModule() throws Exception {
		...
		try {
			...
			extent.log(LogStatus.PASS, "TCAddD2LModule", "Test Pass");
			...
			ExcelUtils.wrtExcelData(126, 3, statusExcel, statusSheet, "Pass");
			ExcelUtils.wrtExcelData(126, 6, statusExcel, statusSheet, "");
		} catch (Exception e) {
			try {
				...
				String statusExcel = Constant.STATUS_EXCEL_FILE;
				String statusSheet = Constant.STATUS_SHEET;
				ExcelUtils.wrtExcelData(126, 3, statusExcel, statusSheet, "Fail");
				ExcelUtils.wrtExcelData(126, 6, statusExcel, statusSheet, e.getMessage());
				Assert.assertTrue(false);
			} catch (FileNotFoundException e1) {
				e1.printStackTrace();
			}
			e.getMessage();
			extent.log(LogStatus.FAIL, "TCAddD2LModule", "Test Fail");
		}
	}
